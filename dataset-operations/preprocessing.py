...
dataset_dir = "caltech"  # Change this to your dataset root folder
...
for sub_dir in ["train", "val"]:
    os.makedirs(os.path.join(output_images_dir, sub_dir), exist_ok=True)
    os.makedirs(os.path.join(output_labels_dir, sub_dir), exist_ok=True)
...
annotation_dir = os.path.join(dataset_dir, "annotations", "annotations", "*")
classes = ['person']
...
for sets in tqdm(sorted(glob.glob(annotation_dir))):
    ...
    # Videos
    for vid_annotations in sorted(glob.glob(os.path.join(sets, "*.vbb"))):
        ...
        for frame_id, obj in enumerate(obj_lists):
            if len(obj) > 0:
                ...
                image_id = f"{set_id}_{video_id}_{frame_id:04d}"
                img_id_list.append(image_id)
                label_list.append(labels)
                split_list.append(split_dataset)
                num_annot_list.append(num_annot)

print("Number of Ground Truth Annotation Boxes:", number_of_truth_boxes)
...
df_caltech_annot.to_csv(os.path.join(csv_output_dir, "frame_metadata.csv"), index=False)
...
for i, set_id in enumerate(set_id_list):
    ...
    for j, vid_id in enumerate(video_id_list):
        ...
        if i <= 5:  # Training Data
            limit = int(round((frame_total / total_train_image) * 37000, 0))
            df_video_id = df_video_id[:limit]
            df_train_filtered = pd.concat([df_train_filtered, df_video_id])
        else:  # Validation Data
            limit = int(round((frame_total / total_val_image) * 15000, 0))
            df_video_id = df_video_id[:limit]
            df_val_filtered = pd.concat([df_val_filtered, df_video_id])

df_train_filtered = df_train_filtered.reset_index(drop=True)
df_val_filtered = df_val_filtered.reset_index(drop=True)

df_train_filtered.to_csv(os.path.join(csv_output_dir, "train_frame_filtered.csv"), index=False)
df_val_filtered.to_csv(os.path.join(csv_output_dir, "val_frame_filtered.csv"), index=False)
